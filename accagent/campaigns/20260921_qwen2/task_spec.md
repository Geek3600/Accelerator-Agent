# Qwen2 FPGA Spatial Accelerator Campaign

Design a complete FPGA-runnable spatial accelerator for the supplied Qwen2 checkpoint and its complete Transformer-block workload. The model architecture, tensor shapes, layer count, sequence length, numeric policy, and checkpoint tensor names must be derived from the supplied model source and checkpoint; do not substitute another model or use fixed Qwen2 dimensions.
