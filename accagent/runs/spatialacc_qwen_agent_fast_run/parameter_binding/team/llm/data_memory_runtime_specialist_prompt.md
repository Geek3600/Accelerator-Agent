...
<task>
Validate that the symbolic bandwidth estimate, AXI beat packing, stream order, edge tensor sizes, and FIFO/default scopes are consistent with the board memory profile and Stage 3 pipeline contracts, while preserving the boundary that Stage 5 owns concrete address assignment.
</task>
...
<subtask>
{
  ...
  "expected_artifacts": [
    "parameter_binding.transfer_count_audit",
    "axi_packing_consistency_notes",
    "fifo_authority_boundary_report"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "parameter_binding.data_memory_runtime_engineer",
  "objective": "Validate that the symbolic bandwidth estimate, AXI beat packing, stream order, edge tensor sizes, and FIFO/default scopes are consistent with the board memory profile and Stage 3 pipeline contracts, while preserving the ...<len=275>",
  "role": "stream and memory interface engineer",
  "title": "Audit AXI packing, transfer counts, and FIFO authority boundaries"
}
</subtask>
