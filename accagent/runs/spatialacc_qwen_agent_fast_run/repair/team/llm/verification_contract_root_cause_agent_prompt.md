<agent>
verification_contract_root_cause_agent
</agent>

<task>
Map each failed checker and contaminated Stage6/Stage7 artifact to the violated verification-plan, hierarchy, tool-protocol, or artifact-contract constraint, distinguishing root causes from downstream symptoms.
</task>
...
<subtask>
{
  ...
  "agent": "verification_contract_root_cause_agent",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    "verification_contract_failure_classification",
    "violated_constraint_map",
    "stage6_repair_or_retry_recommendation",
    "stage7_rerun_prerequisite_list"
  ],
  "handoff_rule": {
    "completion_criteria": [],
    "evidence_rule": "Do not claim pass unless evidence is bound to SACG constraints or explicitly recorded as not_run.",
    "produces": [
      "root_cause_classification_table",
      "contract_repair_scope",
      "blocked_downstream_claims"
    ]
  },
  "handoff_to": [
    "repair.sacg_trust_barrier_engineer",
    "repair.repair_boundary_engineer",
    "repair.final_evidence_gate"
  ],
  "id": "repair.verification_contract_root_cause_engineer",
  "objective": "Map each failed checker and contaminated Stage6/Stage7 artifact to the violated verification-plan, hierarchy, tool-protocol, or artifact-contract constraint, distinguishing root causes from downstream symptoms.",
  "role": "verification contract root-cause engineer",
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "produces": [
        "verification_contract_failure_classification",
        "violated_constraint_map",
        "root_cause_vs_symptom_table",
        "stage6_repair_prerequisites"
      ]
    },
    "decision_authority": [
      "May declare an artifact untrusted when SACG memory truth lists an active contamination barrier.",
      "May recommend Stage6 retry before Stage7 rerun when verification contracts are rejected.",
      "May block downstream promotion claims that depend on rejected producer artifacts."
    ],
    "mission": "Classify the verification-contract root cause behind the repair request without performing free-form design changes.",
    "out_of_scope": [
      "Generating or editing RTL/Chisel/templates.",
      "Running board, AXI, Vivado, or runtime closure.",
      "Treating contaminated Stage6/Stage7 artifacts as validated inputs."
    ],
    "primary_responsibilities": [
      "Read failed invariants verification_plan_static_check and verification_agent_decision_check.",
      "Identify whether the actionable repair belongs to Stage6 contract regeneration, Stage7 rerun sequencing, or Stage8 bounded repair classification.",
      "Trace each failure to SACG constraints and artifact trust state.",
      "Produce a bounded causal map that downstream repair-boundary and evidence-gate agents can consume."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Classify verification-plan and contract failures into bounded repair causes"
}
</subtask>
