...
<task>
Classify each failed checker, rejected artifact, retry request, and contamination barrier into a bounded repair category without treating rejected Stage6/Stage7/Stage8 artifacts as validated design inputs.
</task>
...
<subtask>
{
  ...
  "agent": "verification_repair_agent",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    ...
    "failed_checker_to_constraint_map",
    "contamination_barrier_usage_policy",
    "retry_request_blocker_table"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "repair.stage6_contract_repair_architect",
    "repair.targeted_replay_planner",
    "repair.boundary_authority_gate"
  ],
  "id": "repair.evidence_classifier",
  "objective": "Classify each failed checker, rejected artifact, retry request, and contamination barrier into a bounded repair category without treating rejected Stage6/Stage7/Stage8 artifacts as validated design inputs.",
  "role": "verification evidence classifier",
  "role_assignment": {
    ...
    "decision_authority": [
      "May classify repair causes and blocker priority.",
      "May block handoff if SACG memory truth is inconsistent with proposed repair inputs.",
      "May not approve or execute repairs that modify design artifacts."
    ],
    "mission": "Produce the authoritative repair evidence classification from current SACG memory truth and failed checker evidence.",
    "out_of_scope": [
      "Generating free-form RTL or modifying templates.",
      "Running Stage7 or backend real tools.",
      "Clearing contamination barriers without checker-promoted replacement artifacts."
    ],
    "primary_responsibilities": [
      "Consume candidate_stage_artifact.failures, diagnostics, failed_invariants, active contamination barriers, and open retry requests.",
      "Classify failures as Stage6 contract/provenance repair, Stage7 rerun dependency, Stage8 bounded repair completion, evidence gap, or human-approval-gated action.",
      "Map each class to exact violated constraints and canonical acceptance checkers.",
      "Preserve contamination barriers by marking rejected artifacts as planning/debug evidence only."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Classify failed evidence and active SACG blockers"
}
</subtask>
