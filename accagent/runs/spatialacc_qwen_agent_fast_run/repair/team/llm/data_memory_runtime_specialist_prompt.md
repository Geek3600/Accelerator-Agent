<agent>
data_memory_runtime_specialist
</agent>

<task>
Determine whether missing real-weight manifests, DDR images, and leaf-static reports are repairable by rerunning/generating required Stage7 operator-leaf evidence under the existing Stage6 artifact contract.
</task>
...
<subtask>
{
  ...
  "agent": "data_memory_runtime_specialist",
  "artifact_focus": [
    ...
    "verification/qwen_real_weights/weight_manifest.json",
    "verification/qwen_real_weights/packed_weight_manifest.json",
    "verification/qwen_real_weights/input_manifest.json",
    "verification/qwen_real_weights/input_activation.u32.memh",
    "verification/qwen_real_weights/weight_prefetch.u32.memh",
    "verification/qwen_hierarchy/real_weights.json",
    "verification/qwen_hierarchy/stage_leaf_static.json"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "artifact.repair.real_weight_artifact_gap_report",
    "artifact.repair.leaf_static_gap_report",
    "artifact.repair.targeted_stage7_rerun_plan",
    "artifact.repair.required_manifest_repair_actions"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "repair.real_weight_leaf_artifact_engineer",
  "objective": "Determine whether missing real-weight manifests, DDR images, and leaf-static reports are repairable by rerunning/generating required Stage7 operator-leaf evidence under the existing Stage6 artifact contract.",
  "role": "operator-leaf artifact closure engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May recommend targeted regeneration/rerun of existing contracted evidence artifacts.",
      "May flag contract incompleteness requiring Stage6 backtrack.",
      "May not alter tensor shapes, numeric policy, trusted templates, generated RTL, or board deployment assumptions."
    ],
    "mission": "Constrain the real-weight and leaf-static repair to the smallest evidence-generation slice needed for operator-leaf verification closure.",
    "out_of_scope": [
      "No model-shape reinterpretation or semantic extraction.",
      "No uncontracted simulator execution plan beyond operator-leaf scope.",
      "No board-runtime, bitstream, or Vivado repair actions."
    ],
    "primary_responsibilities": [
      "Consume the root-cause classification matrix, Stage6 verification artifact contract, real-tool node policy, memory layout, and runtime config.",
      "Identify the exact missing manifests/reports and whether each should be produced by case_real_weight_artifacts, case_stage_leaf_static, case_weight_manifest_generate, or a targeted Stage7 rerun.",
      "Check cross-layer consistency between real input/weight manifests, DDR image naming, memory layout, runtime config, and operator-leaf verification scope.",
      "Produce a rerun plan that repairs evidence gaps without expanding to deferred single-layer or board gates."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Bound repair for missing real-weight and leaf-static artifacts"
}
</subtask>
