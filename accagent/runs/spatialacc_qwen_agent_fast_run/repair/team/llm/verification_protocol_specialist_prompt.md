<agent>
verification_protocol_specialist
</agent>

<task>
Analyze the mismatch where later selected operator-leaf real-tool gates appear to pass while the top-level verification_result remains failed, and define a bounded gate-DAG/tool-protocol repair or Stage6 backtrack if contracts are incomplete.
</task>
...
<subtask>
{
  ...
  "action_type": "gate_protocol_reconciliation",
  "agent": "verification_protocol_specialist",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    "artifact.repair.gate_dag_reconciliation_report",
    "artifact.repair.promotion_condition_patch_plan",
    "artifact.repair.retry_or_backtrack_decision",
    "artifact.repair.blocked_scope_statement"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "repair.gate_protocol_reconciliation_engineer",
  "objective": "Analyze the mismatch where later selected operator-leaf real-tool gates appear to pass while the top-level verification_result remains failed, and define a bounded gate-DAG/tool-protocol repair or Stage6 backtrack if con...<len=242>",
  "role": "verification gate-DAG reconciliation engineer",
  "role_assignment": {
    "collaboration_interfaces": {
      "acceptance_checker": "verification_action_audit_check",
      "consumes": [
    ...
    "decision_authority": [
      "May recommend retry versus Stage6 backtrack based on contract completeness.",
      "May define promotion conditions for operator-leaf closure subject to checkers.",
      "May not accept missing evidence, waive real-tool gates, or change human approval policy."
    ],
    "mission": "Resolve checker-order, evidence-ingestion, and promotion-decision ambiguity while preserving hierarchical verification scope boundaries.",
    "out_of_scope": [
      "No generation of missing manifests or testbenches.",
      "No RTL, template, or parameter binding modification.",
      "No promotion beyond operator-leaf closure."
    ],
    "primary_responsibilities": [
      "Consume Stage6 verification plan, gate selector contract, dependency-blocked schema, Stage7 result, and artifact gap reports from peer specialists.",
      "Compare selected gate DAG against actual failure ordering for case_functional_sim_precondition_check, case_real_weight_artifacts, case_stage_leaf_static, case_leaf_functional, and case_leaf_golden_compare.",
      "Define whether the proper action is targeted_failed_checker_rerun, gate-DAG ordering repair, evidence classifier rule repair, or Stage6 backtrack.",
      "Produce a blocked-scope statement prohibiting claims for single-layer, multilayer, AXI/DDR, synthesis, bitstream, and board runtime gates."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Repair checker-order and promotion-decision inconsistencies"
}
</subtask>
