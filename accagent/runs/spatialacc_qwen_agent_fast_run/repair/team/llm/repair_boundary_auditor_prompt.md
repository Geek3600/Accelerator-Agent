<agent>
repair_boundary_auditor
</agent>

<task>
Audit all proposed repair actions for auto-allowed, approval-required, forbidden, retry, or backtrack classification under human-agent boundaries and active SACG contamination barriers.
</task>
...
<subtask>
{
  ...
  "action_type": "repair_boundary_gate",
  "agent": "repair_boundary_auditor",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    "artifact.repair.boundary_approval_matrix",
    "artifact.repair.final_bounded_repair_plan",
    "artifact.repair.sacg_memory_update_plan",
    "artifact.repair.retry_backtrack_request_update"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "repair.boundary_approval_auditor",
  "objective": "Audit all proposed repair actions for auto-allowed, approval-required, forbidden, retry, or backtrack classification under human-agent boundaries and active SACG contamination barriers.",
  "role": "human-boundary and SACG trust-barrier auditor",
  "role_assignment": {
    ...
    "decision_authority": [
      "May block or require approval for any repair action that exceeds bounded evidence repair.",
      "May authorize handoff to stage_retry_request or stage_backtrack_request when checker accepted.",
      "May not execute repair tools, modify design artifacts, or override human boundary constraints."
    ],
    "mission": "Ensure the final repair plan respects human-agent boundaries, SACG memory truth, contamination barriers, and checker-verified design closure.",
    "out_of_scope": [
      "No root-cause reclassification beyond auditing consistency.",
      "No real-tool execution or artifact generation.",
      "No unilateral approval for human-required changes."
    ],
    "primary_responsibilities": [
      "Consume all peer repair reports plus the human-agent boundary and SACG memory truth.",
      "Classify each repair action as auto-allowed, approval-required, forbidden, retry-only, or backtrack-required.",
      "Verify that active contamination barriers on Stage7 artifacts are preserved until accepted rerun evidence clears the failed invariants.",
      "Produce the final handoff package for retry/backtrack request tools and SACG memory update."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Gate bounded repair actions against human approval and contamination policies"
}
</subtask>
