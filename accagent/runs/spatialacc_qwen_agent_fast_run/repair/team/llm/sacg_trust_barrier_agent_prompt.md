<agent>
sacg_trust_barrier_agent
</agent>

<task>
Determine which current SACG-memory blockers prevent promotion and define the trust conditions that must be satisfied before Stage7 evidence can be rerun or reused.
</task>
...
<subtask>
{
  ...
  "action_type": "sacg_memory_blocker_audit",
  "agent": "sacg_trust_barrier_agent",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    ...
    "artifact_trust_matrix",
    "retry_dependency_order",
    "contamination_barrier_resolution_requirements"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "repair.sacg_trust_barrier_engineer",
  "objective": "Determine which current SACG-memory blockers prevent promotion and define the trust conditions that must be satisfied before Stage7 evidence can be rerun or reused.",
  "role": "SACG trust-barrier engineer",
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "downstream_consumers": [
        "repair.repair_boundary_engineer",
        "repair.final_evidence_gate",
        "stage6.verification_artifacts retry flow",
        "stage7.verification rerun flow"
      ],
      "produces": [
        "artifact_trust_matrix",
        "retry_dependency_order",
        "barrier_resolution_requirements"
      ]
    },
    "decision_authority": [
      "May block reuse of contaminated artifacts as design inputs.",
      "May order repair dependencies according to active retry requests.",
      "May require a clean Stage6 contract before Stage7 rerun."
    ],
    "mission": "Protect cross-layer consistency by enforcing current SACG memory truth during repair classification.",
    "out_of_scope": [
      "Closing barriers without checker evidence.",
      "Ignoring active blockers because real-tool operator-leaf evidence passed.",
      "Approving backend or deployment progress."
    ],
    "primary_responsibilities": [
      "Use only state_summary.sacg_memory_truth to identify active blockers.",
      "Classify Stage6, Stage7, and Stage8 artifacts as validated, planning/debug-only, or blocked.",
      "Specify which retry request must be resolved first and which artifacts cannot be consumed as trusted inputs.",
      "Provide trust-barrier evidence to the final repair gate."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Audit active retry requests and contamination barriers for repair legality"
}
</subtask>
