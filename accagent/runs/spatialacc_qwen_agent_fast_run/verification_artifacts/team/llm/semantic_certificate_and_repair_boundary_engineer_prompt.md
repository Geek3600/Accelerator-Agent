<agent>
semantic_certificate_and_repair_boundary_engineer
</agent>

<task>
Define a unified current-contract certificate-validation and failure-routing policy across operator-leaf, single-layer, and board AXI/DDR closures. Treat the two present lower-layer certificates as non-reusable until validated under spatialaccagent.real_weight_semantic_evidence.v2, prevent unconditional reopening of passed layers, and require boundary-trace contradiction plus targeted replay before a higher-layer failure can trigger lower-layer repair.
</task>
...
<subtask>
{
  ...
  "action_type": "certificate_validation_and_repair_boundary_refinement",
  "agent": "semantic_certificate_and_repair_boundary_engineer",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    ...
    "artifact.stage6.cross_layer_failure_route_matrix_refined",
    "artifact.stage6.targeted_backtrack_evidence_contract",
    "artifact.stage6.repair_contamination_prevention_rules"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "verification_artifacts.repair_certificate_specialist",
  "objective": "Define a unified current-contract certificate-validation and failure-routing policy across operator-leaf, single-layer, and board AXI/DDR closures. Treat the two present lower-layer certificates as non-reusable until val...<len=456>",
  "role": "semantic certificate and bounded-repair engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May deny certificate reuse when contract version, identity, evidence scope, or real-weight proof is stale or missing.",
      "May authorize a targeted lower-layer replay when a higher-layer trace supplies a concrete contradiction.",
      "May not create speculative SACG blockers or reopen a passed layer without checker evidence."
    ],
    "mission": "Ensure that verification evidence is reusable only under the current semantic contract and that failures produce minimal, checker-grounded repair handoffs rather than broad RTL churn.",
    "out_of_scope": [
      "Free-form RTL debugging",
      "Changing templates or architecture without a localized repair cause",
      "Executing backend or board tools"
    ],
    "primary_responsibilities": [
      "Consume all three layer refinements, immutable identity contracts, certificate summaries, debug schemas, and SACG memory truth.",
      "Specify current-contract validation for present and future promotion certificates.",
      "Map each failed gate to violated constraints, earliest causal boundary, targeted replay, failed-checker rerun, and bounded repair owner.",
      "Prevent stale, cross-run, sampled-weight, or smoke-only evidence from crossing promotion boundaries.",
      "Distinguish ordinary downstream obligations from actual retry, backtrack, or contamination blockers."
    ]
  },
  "role_profile": {
    "capabilities": [
      ...
      "validate certificate reuse predicates",
      "prevent cross-run evidence contamination"
    ],
    "constraint_focus": [
      "constraint.verification.plan",
      "constraint.verification.hierarchy",
  ...
  "title": "Refine certificate reuse, backtrack, and repair boundaries"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.repair_certificate_specialist",
