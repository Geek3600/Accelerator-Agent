<agent>
single_layer_integration_verification_engineer
</agent>

<task>
Refine the second repair-loop layer for one connected transformer decoder block. Require current operator-leaf semantic promotion, immutable identity continuity across self-attention, residual, normalization, and MLP boundaries, a reproducible testbench scaffold, real functional simulation, target-model golden comparison, complete block-weight consumption, and a current-contract single-layer semantic certificate before any multi-layer or board-wrapper gate is eligible.
</task>
...
<subtask>
{
  ...
  "action_type": "connected_kernel_verification_planning",
  "agent": "single_layer_integration_verification_engineer",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    ...
    "artifact.stage6.single_layer_boundary_identity_matrix",
    "artifact.stage6.single_layer_current_contract_revalidation_plan",
    "artifact.stage6.single_layer_failure_to_repair_routes"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.board_axi_ddr_gate_specialist",
    "verification_artifacts.repair_certificate_specialist",
    "verification_artifacts.canonical_gate_dag_auditor"
  ],
  "id": "verification_artifacts.single_layer_gate_specialist",
  "objective": "Refine the second repair-loop layer for one connected transformer decoder block. Require current operator-leaf semantic promotion, immutable identity continuity across self-attention, residual, normalization, and MLP bou...<len=473>",
  "role": "connected transformer-layer integration verification engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May block board-wrapper eligibility when single-layer evidence is incomplete, stale, identity-mismatched, or semantically inconsistent.",
      "May request targeted lower-layer replay only with a contradictory current-layer boundary trace.",
      "May not authorize multi-layer, backend, bitstream, or board-runtime promotion."
    ],
    "mission": "Specify checker-verified closure for exactly one connected transformer layer without bypassing leaf promotion or weakening GQA, RoPE, residual, stream-order, or numeric contracts.",
    "out_of_scope": [
      "Per-leaf RTL modification",
      "Vivado synthesis, implementation, or bitstream generation",
      "Board runtime execution"
    ],
    "primary_responsibilities": [
      "Consume the operator-leaf refinement, pipeline graph, parameter bindings, generated kernel, target-model reference contract, and current semantic identity contract.",
      "Check connected operator order, tensor shapes, GQA head mapping, residual precision, cast points, transfer counts, and boundary identities.",
      "Define the exact dependencies among case_tb_scaffold, single_transformer_layer, functional simulation, golden comparison, and semantic aggregation.",
      "Specify current-contract revalidation for the stale single-layer promotion certificate.",
      "Produce failure routes that distinguish leaf contradiction from connected-boundary or integration defects."
    ]
  },
  "role_profile": {
    "capabilities": [
      ...
      "audit connected-kernel boundary identity"
    ],
    "constraint_focus": [
      "constraint.model.decoder",
      "constraint.shape.model",
  ...
  "title": "Refine the connected single-layer kernel gate sub-DAG"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.single_layer_gate_specialist",
