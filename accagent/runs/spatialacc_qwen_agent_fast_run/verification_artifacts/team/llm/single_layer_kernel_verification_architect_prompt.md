<agent>
single_layer_kernel_verification_architect
</agent>

<task>
Audit the second repair-loop layer from promoted leaf evidence through the connected transformer-layer kernel, functional simulation, semantic evidence, and golden comparison. Resolve gate identity and dependency consistency between case_single_transformer_layer and single_transformer_layer, and verify that attention, residual, MLP, numeric-cast, stream-order, and complete-weight contracts are preserved across the connected layer.
</task>
...
<subtask>
{
  ...
  "agent": "single_layer_kernel_verification_architect",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    "artifact.stage6.single_layer_gate_slice_review",
    "artifact.stage6.single_layer_gate_identity_binding",
    "artifact.stage6.single_layer_certificate_reuse_decision"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "verification_artifacts.single_layer_gate_dag_specialist",
  "objective": "Audit the second repair-loop layer from promoted leaf evidence through the connected transformer-layer kernel, functional simulation, semantic evidence, and golden comparison. Resolve gate identity and dependency consist...<len=434>",
  "role": "connected transformer-layer verification engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May approve reuse of the existing single-layer promotion certificate when current bindings and hashes agree.",
      "May reject ambiguous gate aliases or missing merge dependencies.",
      "May request targeted lower-layer replay only with a current contradictory boundary trace; may not initiate broad RTL repair."
    ],
    "mission": "Ensure the connected transformer-layer kernel is a semantically faithful, template-constrained composition of promoted leaves with real-tool evidence suitable for third-layer consumption.",
    "out_of_scope": [
      "Independent leaf-module reimplementation",
      "Board shell, AXI, or DDR integration",
      "Vivado implementation, bitstream generation, and board runtime"
    ],
    "primary_responsibilities": [
      "Verify merge ordering for RMSNorm, causal GQA attention, residual paths, gated MLP, and final residual addition.",
      "Audit GQA head mapping, shape compatibility, numeric cast points, and stream ordering across merge boundaries.",
      "Bind case_single_transformer_layer and single_transformer_layer to an explicit canonical producer/checker relationship.",
      "Validate that functional and golden evidence uses target-model references and complete checkpoint-derived weights.",
      "Define the precise trace condition that can invalidate otherwise reusable leaf evidence."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Refine the connected single-layer real-tool gate slice"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.single_layer_gate_dag_specialist",
