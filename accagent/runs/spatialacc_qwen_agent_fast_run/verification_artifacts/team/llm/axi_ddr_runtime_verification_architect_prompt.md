...
<task>
Refine and audit the AXI/DDR and runtime-interface portion of the verification gate DAG so board wrapper, address mapping, DDR image roundtrip, and runtime ABI gates are consistent with SACG memory/runtime constraints and only execute after multilayer functional closure.
</task>
...
<subtask>
{
  ...
  "agent": "axi_ddr_runtime_verification_architect",
  "artifact_focus": [
    "artifact.stage5.memory_layout",
    "artifact.stage5.runtime_config",
    "artifact.stage5.generated_code_package AXI/DDR top wrapper",
    "verification_gate_dag nodes: case_board_interface_discovery, case_axi_ddr_interface, case_axi_protocol_check, case_ddr_image_roundtrip, case_runtime_abi_check",
    "required_tool_protocols for case_board_interface_discovery, case_axi_ddr_interface, case_axi_protocol_check, case_ddr_image_roundtrip, case_runtime_abi_check"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    ...
    "runtime_memory_layout_crosscheck_plan",
    "ddr_image_roundtrip_acceptance_contract",
  ...
  "id": "verification_artifacts.axi_ddr_runtime_verification_architect",
  "objective": "Refine and audit the AXI/DDR and runtime-interface portion of the verification gate DAG so board wrapper, address mapping, DDR image roundtrip, and runtime ABI gates are consistent with SACG memory/runtime constraints an...<len=271>",
  "role": "AXI/DDR runtime interface verification architect",
  "role_assignment": {
    ...
    "decision_authority": [
      "May approve AXI/DDR handoff only when runtime and memory evidence gates are canonical and ordered after functional closure.",
      "May block backend handoff on address-map, transfer-count, DDR roundtrip, or ABI ambiguity.",
      "May not waive real AXI/DDR runtime requirements or replace them with smoke-only evidence."
    ],
    "mission": "Ensure runtime wrapping and DDR evidence gates preserve stream order, address correctness, transfer completeness, and ABI compatibility.",
    "out_of_scope": [
      "Changing board profile, memory layout schema, or runtime ABI without a bounded repair request.",
      "Running Vivado or physical board runtime.",
      "Claiming final deployment pass or functional correctness."
    ],
    "primary_responsibilities": [
      "Consume Stage 5 memory_layout, runtime_config, generated AXI/DDR wrapper, and multilayer promotion contract.",
      "Validate dependency ordering for board interface discovery, AXI/DDR interface, protocol checks, DDR image roundtrip, and runtime ABI checks.",
      "Cross-check planned acceptance with addr_map_check, transfer_count_check, and data_order_trace_check.",
      "Identify missing tool protocol or artifact-path gaps as planned_tool/planned_checker requirements if any are absent.",
      "Produce a pre-backend AXI/DDR runtime contract for backend/board specialists."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Validate AXI/DDR wrapper, memory layout, runtime ABI, and DDR transfer evidence gates"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.axi_ddr_runtime_verification_architect",
