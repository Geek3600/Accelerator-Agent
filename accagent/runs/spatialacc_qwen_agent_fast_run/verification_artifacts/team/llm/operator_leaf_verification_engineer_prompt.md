<agent>
operator_leaf_verification_engineer
</agent>

<task>
Refine the first repair-loop layer for all nine declared spatial operator stages. Bind generated leaf modules to the current templates, parameter bindings, complete checkpoint-derived weights, target-model references, semantic testbenches, boundary contracts, functional simulation, numeric comparison, and a current-contract semantic promotion certificate. Explicitly invalidate presence-only reuse of artifact.stage7.operator_leaf_promotion_certificate because current_contract_valid=false.
</task>
...
<subtask>
{
  ...
  "agent": "operator_leaf_verification_engineer",
  "artifact_focus": [
    ...
    "leaf_stage.stage_00_rms_norm_1 through leaf_stage.stage_08_residual_add_2"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    ...
    "artifact.stage6.operator_leaf_current_contract_revalidation_plan",
    "artifact.stage6.operator_leaf_evidence_identity_contract",
    "artifact.stage6.operator_leaf_repair_boundary_map"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.verification_dag_evidence_auditor"
  ],
  "id": "verification_artifacts.operator_leaf_gate_specialist",
  "objective": "Refine the first repair-loop layer for all nine declared spatial operator stages. Bind generated leaf modules to the current templates, parameter bindings, complete checkpoint-derived weights, target-model references, se...<len=492>",
  "role": "operator-leaf semantic verification engineer",
  "role_assignment": {
    "collaboration_interfaces": {
      "acceptance_checker": "case_operator_leaf_semantic_evidence",
      "consumes": [
        "artifact.stage2.template_selection",
        "artifact.stage3.pipeline_plan",
        "artifact.stage4.parameter_binding",
    ...
    "decision_authority": [
      "May reject an operator-leaf promotion edge when semantic, identity, weight-consumption, numeric, or tool evidence is incomplete.",
      "May require targeted leaf re-execution or bounded template repair.",
      "May not approve single-layer, board-wrapper, backend, bitstream, or board-runtime promotion."
    ],
    "mission": "Produce a checker-verifiable operator-leaf gate subgraph that proves current template identity, complete real-weight consumption, numeric correctness, and boundary-localized repairability before any layer merge.",
    "out_of_scope": [
      "Free-form replacement RTL generation",
      "Connected single-layer or board AXI/DDR acceptance",
      "Vivado synthesis, implementation, bitstream generation, or board runtime"
    ],
    "primary_responsibilities": [
      "Consume Stage 2 through Stage 6 template, pipeline, parameter, generated-package, and verification contracts.",
      "Audit all nine leaf nodes without inventing model-specific core names beyond the supplied case adapter.",
      "Bind each leaf to case_real_weight_artifacts, case_target_model_reference, case_semantic_testbench, functional simulation, golden comparison, and immutable artifact identity.",
      "Specify current-contract certificate regeneration or validation because the existing certificate is stale for the current contract.",
      "Produce failure routes that identify the earliest violated leaf boundary and preserve passed unrelated leaves."
    ]
  },
  "role_profile": {
    "capabilities": [
      "classify operator-leaf evidence",
      "map semantic or numeric failures to violated SACG constraints",
      "enforce template and repair boundaries"
    ],
    "constraint_focus": [
      "constraint.verification.plan",
      "constraint.verification.hierarchy",
  ...
  "title": "Refine the operator-leaf real-tool closure subgraph"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.operator_leaf_gate_specialist",
