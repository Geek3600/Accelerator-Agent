<agent>
single_layer_kernel_verification_engineer
</agent>

<task>
Refine the second-layer DAG that merges the verified attention and MLP paths into one connected decoder-block kernel. Verify operator order, residual paths, stream and cast continuity, complete checkpoint-derived tensor consumption, functional simulation, golden comparison, and hash-consistent reuse of the existing single-layer promotion certificate before any multi-layer or board-wrapper node may execute.
</task>
...
<subtask>
{
  ...
  "action_type": "integration_evidence_review",
  "agent": "single_layer_kernel_verification_engineer",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    "single_layer_gate_dependency_audit.json",
    "single_layer_boundary_and_cast_continuity_report.json",
    "single_layer_promotion_evidence_reuse_decision.json",
    "single_layer_repair_boundary_manifest.json"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "verification_artifacts.single_layer_dag_specialist",
  "objective": "Refine the second-layer DAG that merges the verified attention and MLP paths into one connected decoder-block kernel. Verify operator order, residual paths, stream and cast continuity, complete checkpoint-derived tensor ...<len=409>",
  ...
  "role_assignment": {
    ...
    "decision_authority": [
      "Own the single-layer promotion-readiness verdict.",
      "Block case_multilayer_pipeline on missing or contradictory evidence.",
      "Authorize leaf backtrack only with current boundary-trace support."
    ],
    "mission": "Audit the current connected decoder-block kernel and its gate dependencies without duplicating accepted leaf verification.",
    "out_of_scope": [
      "Operator-leaf reimplementation.",
      "AXI/DDR wrapper construction.",
      "Backend synthesis and deployment."
    ],
    "primary_responsibilities": [
      "Verify merge.self_attention and merge.mlp prerequisites.",
      "Check residual and numeric cast boundaries through merge.single_decoder_block.",
      "Validate the existing single-layer certificate against current artifact hashes.",
      "Produce a bounded failure slice when evidence conflicts."
    ]
  },
  "role_profile": {
    "collaboration_interfaces": {
      "acceptance_checker": "case_single_layer_semantic_evidence",
      "consumes": [
        "leaf promotion evidence",
        "pipeline plan",
        "parameter bindings",
        "single-layer tool evidence"
      ],
      ...
      "produces": [
        "single-layer dependency audit",
        "continuity report",
        "promotion reuse decision",
        "repair manifest"
      ]
    },
    "decision_authority": [
      "May accept or block second-layer promotion.",
      "May classify a failure as leaf contradiction, merge defect, harness defect, or evidence defect.",
      "May prevent multi-layer execution until second-layer closure is complete."
    ],
    "family": "model_shape",
    "mission": "Preserve model semantics and dataflow continuity across the connected single transformer-layer kernel.",
    "out_of_scope": [
      "Changing model configuration or GQA semantics.",
      "Generating unrelated accelerator blocks.",
      "Vivado, bitstream, or board-runtime execution."
    ],
    "primary_responsibilities": [
      "Audit decoder-block operator and residual ordering.",
      "Check shape, GQA head mapping, stream width, and cast continuity at merge boundaries.",
      "Validate target-reference and real-weight evidence for the connected kernel.",
      "Constrain repairs to the earliest violated integration boundary."
    ]
  },
  "title": "Audit connected single-layer kernel closure and promotion boundary"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.single_layer_dag_specialist",
