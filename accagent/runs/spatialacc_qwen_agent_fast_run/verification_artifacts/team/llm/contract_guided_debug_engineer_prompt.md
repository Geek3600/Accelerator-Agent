<agent>
contract_guided_debug_engineer
</agent>

<task>
Audit the cross-cutting debug contract for all three verification layers. Ensure every failed real-tool node produces a case-adapter-independent boundary trace, earliest-violation localization, targeted replay plan, violated SACG constraint mapping, and minimal repair context; preserve prior promotion evidence unless contradicted by a current bound trace.
</task>
...
<subtask>
{
  ...
  "action_type": "repair_boundary_contract_review",
  "agent": "contract_guided_debug_engineer",
  "artifact_focus": [
    ...
    "three-layer promotion and backtrack policy"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "three_layer_failure_routing_matrix.json",
    "debug_trace_contract_audit.json",
    "targeted_replay_handoff_schema.json",
    "repair_and_backtrack_authority_matrix.json"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "verification_artifacts.repair_contract_specialist",
  "objective": "Audit the cross-cutting debug contract for all three verification layers. Ensure every failed real-tool node produces a case-adapter-independent boundary trace, earliest-violation localization, targeted replay plan, viol...<len=357>",
  "role": "failure-localization and bounded-repair engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "Own repair-contract acceptance.",
      "Block promotion on missing causal context.",
      "Do not approve symptom-only patches."
    ],
    "mission": "Validate repair handoffs and backtrack boundaries shared by the three verification layers.",
    "out_of_scope": [
      "Executing board runtime.",
      "Changing templates or parameters directly.",
      "Creating backtrack requests without current checker evidence."
    ],
    "primary_responsibilities": [
      "Check required trace fields and monitor points.",
      "Define checker-to-repair routing.",
      "Ensure prior pass evidence is reusable but challengeable by current traces.",
      "Declare a planned tool dependency only when the deployed protocol lacks a required capability."
    ]
  },
  "role_profile": {
    "collaboration_interfaces": {
      "acceptance_checker": "causal_repair_context_check",
      "consumes": [
        "debug schemas",
        "gate DAGs",
        "tool evidence",
    ...
    "decision_authority": [
      "May reject an unlocalized failure handoff.",
      "May authorize targeted replay or bounded backtrack when evidence supports it.",
      "May require a planned tool implementation if a required localization capability is absent."
    ],
    "family": "verification_repair",
    "mission": "Prevent repair-loop spin by turning checker failures into compact, constraint-bound causal repair handoffs.",
    "out_of_scope": [
      "Generic RTL debugging.",
      "Unbounded architecture redesign.",
      "Treating historical SACG records as active blockers."
    ],
    "primary_responsibilities": [
      "Audit boundary trace completeness and portability.",
      "Map failed checkers to violated SACG constraints and repair owners.",
      "Enforce targeted replay before lower-layer backtrack.",
      "Separate template, pipeline, memory/runtime, and backend repair domains."
    ]
  },
  "title": "Audit anti-spin debug, targeted replay, and repair handoff contracts"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.repair_contract_specialist",
