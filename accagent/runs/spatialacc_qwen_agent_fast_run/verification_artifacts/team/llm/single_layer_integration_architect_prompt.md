<agent>
single_layer_integration_architect
</agent>

<task>
Refine the second repair-loop layer for the connected self-attention, residual, normalization, and MLP path, ensuring that only current-contract-valid operator-leaf evidence can feed single-layer construction, real functional simulation, golden comparison, and semantic promotion.
</task>
...
<subtask>
{
  ...
  "action_type": "integration_gate_dag_refinement",
  "agent": "single_layer_integration_architect",
  "artifact_focus": [
    ...
    "case_single_layer_functional",
    "case_single_layer_golden_compare",
    "single-layer promotion certificate contract"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "single-layer gate dependency map",
    "connected boundary and operator-order contract",
    "single-layer real-weight and numeric-comparison evidence contract",
    "single-layer promotion and targeted-backtrack rules"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.verification_evidence_gate_auditor"
  ],
  "id": "verification_artifacts.single_layer_integration_architect",
  "objective": "Refine the second repair-loop layer for the connected self-attention, residual, normalization, and MLP path, ensuring that only current-contract-valid operator-leaf evidence can feed single-layer construction, real funct...<len=280>",
  "role": "single transformer-layer integration verification architect",
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "consumes": [
        "operator-leaf contract",
        "Stage 3 through Stage 5 artifacts",
        "model reference and semantic testbench contracts"
      ],
      ...
      "produces": [
        "single-layer DAG fragment",
        "connected boundary contract",
        "single-layer promotion rules"
      ]
    },
    "decision_authority": [
      "May reject inconsistent merge ordering or stale leaf certificates.",
      "May require targeted boundary replay for the earliest connected-layer violation.",
      "May not authorize board wrapper, synthesis, bitstream, or board runtime."
    ],
    "mission": "Own cross-operator semantic and stream consistency for the connected single transformer-layer verification gate.",
    "out_of_scope": [
      "Editing operator templates without a localized violation",
      "Board AXI/DDR integration",
      "Vivado and physical implementation"
    ],
    "primary_responsibilities": [
      "Audit GQA head mapping, RoPE, causal ordering, residual placement, and MLP sequencing.",
      "Verify numeric cast and residual precision bindings across all connected boundaries.",
      "Require current-valid leaf promotion evidence before layer construction or simulation.",
      "Define evidence-triggered targeted backtracking rather than unconditional reopening of leaf gates."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Refine connected single-layer closure gates"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.single_layer_integration_architect",
