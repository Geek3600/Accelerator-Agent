<agent>
operator_leaf_verification_architect
</agent>

<task>
Refine the first repair-loop layer for all nine declared spatial operator stages. Bind each leaf to its selected template, parameter and numeric policy, complete real-weight tensor consumption, target-model boundary reference, semantic testbench, boundary trace contract, functional simulation, golden comparison, and one checker-bound semantic promotion aggregate. Explicitly invalidate reuse of the stale operator-leaf certificate until current-contract evidence is regenerated and validated.
</task>
...
<subtask>
{
  ...
  "agent": "operator_leaf_verification_architect",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    ...
    "artifact.stage6.operator_leaf_failure_to_repair_routes",
    "operator-leaf evidence manifest requirements for all nine stages"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.single_layer_gate_specialist",
  ...
  "objective": "Refine the first repair-loop layer for all nine declared spatial operator stages. Bind each leaf to its selected template, parameter and numeric policy, complete real-weight tensor consumption, target-model boundary refe...<len=494>",
  "role": "operator-leaf verification and template-binding engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May reject a leaf promotion edge when any required semantic or real-weight dependency is absent or stale.",
      "May require targeted replay before a passed lower-level result is reopened.",
      "May not change model semantics, numeric policy, selected templates, or generated RTL outside an approved repair route."
    ],
    "mission": "Produce a template-constrained, checker-verifiable operator-leaf gate plan that cannot promote sampled weights, stale identities, or smoke-only simulation.",
    "out_of_scope": [
      "Executing Vivado synthesis or implementation",
      "Building the AXI/DDR board wrapper",
      "Changing the target model, checkpoint, or numeric policy"
    ],
    "primary_responsibilities": [
      "Consume the selected templates, bound parameters, generated package, current semantic identity contract, and nine declared leaf-stage definitions.",
      "Verify that each leaf gate binds model shape, numeric tolerance, real checkpoint tensors, DUT weight consumption, trace schema, and generated semantic testbench.",
      "Define fan-out and aggregation dependencies without weakening bottom-up promotion ordering.",
      "Map each leaf failure to targeted replay and a bounded template, parameter, or pipeline repair boundary.",
      "Produce current-contract certificate revalidation inputs for downstream single-layer closure."
    ]
  },
  "role_profile": {
    "capabilities": [
      ...
      "classify leaf-level evidence",
      "map failures to template or parameter bindings"
    ],
    "constraint_focus": [
      "constraint.numeric.policy",
      "constraint.template.library",
  ...
  "title": "Refine the operator-leaf real-tool gate sub-DAG"
}
</subtask>
