<agent>
operator_leaf_evidence_architect
</agent>

<task>
Refine the operator-leaf repair-loop gate DAG for all nine generated stages, ensuring template-bound module coverage, boundary contracts, real functional simulation, golden comparison, and targeted repair slicing are explicit before layer merge.
</task>
...
<subtask>
{
  ...
  "agent": "operator_leaf_evidence_architect",
  "artifact_focus": [
    ...
    "gates:case_stage_leaf_static,boundary_contract_check,case_leaf_functional,case_leaf_golden_compare"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "operator_leaf_gate_matrix.json",
    "leaf_boundary_contract_map.json",
    "leaf_failure_to_constraint_map.json",
    "operator_leaf_evidence_ledger.json"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.single_layer_merge_planner",
    "verification_artifacts.repair_boundary_evidence_auditor"
  ],
  "id": "verification_artifacts.operator_leaf_evidence_architect",
  "objective": "Refine the operator-leaf repair-loop gate DAG for all nine generated stages, ensuring template-bound module coverage, boundary contracts, real functional simulation, golden comparison, and targeted repair slicing are exp...<len=245>",
  ...
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "consumes": [
        ...
        "candidate_stage_artifact.hierarchical_verification.stage_agents",
        "candidate_stage_artifact.verification_artifact_contract.debug_closure_contract"
      ],
      ...
      "produces": [
        ...
        "leaf_failure_to_constraint_map",
        "leaf_evidence_path_requirements"
      ]
    },
    "decision_authority": [
      "May require additional leaf-stage evidence when a stage lacks module boundary or numeric-contract coverage.",
      "May block promotion from operator_leaf_modules to single_transformer_layer_kernel if any required leaf gate is missing or pending.",
      "May classify failures to operator-local repair boundaries but may not authorize template rewrites outside bounded repair policy."
    ],
    "mission": "Define and audit the bottom repair-loop layer for all operator leaf modules so each generated template-bound stage has static boundary, functional simulation, golden comparison, and failure-slice evidence before any merge is promoted.",
    "out_of_scope": [
      "Do not run Vivado synthesis, implementation, bitstream, or board runtime.",
      "Do not change model semantics, numeric policy, or selected templates.",
      "Do not claim final verification pass; only define and audit required leaf-layer gates."
    ],
    "primary_responsibilities": [
      "Consume Stage 3 pipeline plan, Stage 4 parameter bindings, Stage 5 generated code package, and the candidate hierarchical_verification.stage_agents list.",
      "Group the nine leaf stages by operator family while preserving stage_id, op, module_checks, numeric bit widths, ready/valid stream boundaries, and transfer-unit ordering.",
      "Bind required real-tool evidence gates case_stage_leaf_static, boundary_contract_check, case_leaf_functional, and case_leaf_golden_compare to every leaf stage without accepting smoke-only or default-weight evidence.",
      "Define how leaf failures map to violated SACG constraints and compact repair context fields rather than free-form RTL debugging.",
      "Produce a leaf-level verification sub-DAG and evidence ledger that downstream merge planners can consume."
    ],
  ...
  "title": "Plan operator leaf module verification gates"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.operator_leaf_evidence_architect",
