<agent>
hierarchical_verification_evidence_architect
</agent>

<task>
Merge the four specialist refinements into one acyclic, checker-bound, template-constrained verification DAG. Audit complete cross-layer dependencies, current-contract certificate validity, tool protocol bindings, artifact identities, failure routes, and the strict cut between board functional closure and downstream Vivado/board-runtime work. Produce the final Stage 6 refinement only if every required verification node has a configured producer and canonical checker.
</task>
...
<subtask>
{
  ...
  "action_type": "checker_verified_dag_synthesis",
  "agent": "hierarchical_verification_evidence_architect",
  "artifact_focus": [
    ...
    "all parallel_group=0 refinement artifacts",
  ...
  "expected_artifacts": [
    "artifact.stage6.refined_three_layer_verification_gate_dag",
    "artifact.stage6.current_contract_certificate_reexecution_manifest",
    "artifact.stage6.verification_to_backend_handoff_contract",
    "artifact.stage6.refined_stage7_gate_selector_contract",
    "artifact.stage6.refinement_audit_report"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "stage7.verification",
    "downstream backend-board planning after case_board_semantic_evidence passes"
  ],
  "id": "verification_artifacts.verification_dag_evidence_auditor",
  "objective": "Merge the four specialist refinements into one acyclic, checker-bound, template-constrained verification DAG. Audit complete cross-layer dependencies, current-contract certificate validity, tool protocol bindings, artifa...<len=471>",
  "role": "hierarchical verification evidence-gate architect",
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "downstream_consumers": [
        ...
        "downstream backend-board stage"
      ],
      "handoff_to": [
        "stage7.verification",
        "downstream backend-board planning after case_board_semantic_evidence passes"
    ...
    "decision_authority": [
      "May accept or reject the refined Stage 6 verification DAG.",
      "May block Stage 7 execution when any required node lacks a configured producer, canonical checker, or valid prerequisite certificate.",
      "May not waive real-weight semantic evidence, functional simulation, repair-boundary policy, or final board-runtime obligations."
    ],
    "mission": "Serve as the evidence gate for a SACG-guided three-layer verification plan and release only a checker-complete DAG suitable for real-tool Stage 7 execution.",
    "out_of_scope": [
      "Executing Vivado synthesis or implementation in this stage",
      "Generating a bitstream or running the physical board",
      "Free-form redesign outside template-constrained repair boundaries"
    ],
    "primary_responsibilities": [
      "Consume every group-0 refinement plus current Stage 3 through Stage 7 evidence and SACG memory truth.",
      "Check DAG acyclicity, bottom-up maturity ordering, exact producer/checker bindings, and immutable evidence identity propagation.",
      "Require re-execution or current-contract validation for stale operator-leaf and single-layer certificates.",
      "Ensure functional_sim is the canonical source for all third-layer classifiers and that board semantic aggregation gates backend entry.",
      "Keep case_vivado_synthesis, case_vivado_implementation, case_runtime_bitstream, case_runtime_abi_check, and board_runtime downstream rather than treating them as current verification closure."
    ]
  },
  "role_profile": {
    "capabilities": [
      "classify and aggregate evidence",
      "audit hierarchical gate dependencies",
      "block unsupported promotion claims"
    ],
    "constraint_focus": [
      "constraint.verification.plan",
      "constraint.verification.hierarchy",
  ...
  "title": "Reconcile and certify the three-layer verification gate DAG"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.verification_dag_evidence_auditor",
