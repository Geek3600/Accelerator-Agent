<agent>
repair_boundary_evidence_auditor
</agent>

<task>
Perform the evidence-gate audit over all specialist sub-DAGs, ensuring canonical checker grounding, bottom-up promotion rules, downstream-only backend/board obligations, and bounded repair handoff fields are complete.
</task>
...
<subtask>
{
  ...
  "agent": "repair_boundary_evidence_auditor",
  "artifact_focus": [
    "integrated verification_gate_dag",
    "maturity_contract.promotion_rule",
    "checker_results.pending_downstream",
    "required_tool_protocols",
    "debug_closure_contract.repair_handoff_required_fields",
    "sacg_memory_truth:no_active_blockers"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "final_verification_gate_dag.json",
    "checker_evidence_acceptance_matrix.json",
    "repair_boundary_handoff_contract.json",
    "downstream_real_tool_execution_manifest.json",
    "stage6_promotion_decision.json"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "Stage7 real-tool verification execution",
    "Stage8 bounded repair loop",
    "backend_board downstream stage"
  ],
  "id": "verification_artifacts.repair_boundary_evidence_auditor",
  "objective": "Perform the evidence-gate audit over all specialist sub-DAGs, ensuring canonical checker grounding, bottom-up promotion rules, downstream-only backend/board obligations, and bounded repair handoff fields are complete.",
  "role": "verification evidence gate auditor",
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "consumes": [
        ...
        "candidate_stage_artifact.verification_artifact_contract",
      ...
      "produces": [
        "final_verification_gate_dag",
        "checker_evidence_acceptance_matrix",
        "repair_boundary_handoff_contract",
        "downstream_real_tool_execution_manifest",
        "stage6_promotion_decision"
      ]
    },
    "decision_authority": [
      "May reject any sub-DAG that bypasses lower-layer pass evidence or reopens passed lower layers without contradicting boundary trace.",
      "May require planned_tool or planned_checker implementation if a needed capability is absent from the registry.",
      "May approve Stage 6 verification_artifacts promotion only as a plan/contract gate, not as functional, bitstream, or board closure."
    ],
    "mission": "Aggregate specialist outputs into one checker-verifiable real-tool verification DAG and audit that promotion, repair, and downstream implementation boundaries obey SACG policy and human-boundary constraints.",
    "out_of_scope": [
      "Do not execute real simulator, Vivado, or board runtime tools in this decomposition stage.",
      "Do not perform marketing, paper writing, generic brainstorming, or unrelated cleanup.",
      "Do not override active SACG memory blockers; if blockers appear, promotion must be stopped."
    ],
    "primary_responsibilities": [
      "Consume leaf, single-layer, and board AXI/DDR sub-DAGs and check their combined ordering against the candidate verification_gate_dag phase_order and maturity_contract.",
      "Verify all checker/tool names use canonical action_grounding_registry entries or explicitly declare planned_tool/planned_checker gaps if missing.",
      "Enforce no final pass claims while deadlock_watchdog, implementation_package_static, board_runtime, Vivado, or runtime bitstream evidence remains downstream pending.",
      "Audit repair routing fields for failure_localization_check, targeted_replay_check, causal_repair_context_check, and repair_boundary_check compatibility.",
      "Produce the final Stage 6 evidence-gate DAG artifact and downstream handoff contract for Stage 7 real-tool execution."
    ],
  ...
  "title": "Audit integrated verification DAG and repair boundaries"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.repair_boundary_evidence_auditor",
