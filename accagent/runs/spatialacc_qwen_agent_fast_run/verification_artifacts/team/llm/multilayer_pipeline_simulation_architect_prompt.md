<agent>
multilayer_pipeline_simulation_architect
</agent>

<task>
Refine the third-layer multi-layer pipeline branch so that case_multilayer_pipeline is constructed only from promoted single-layer evidence and its functionality, ordering, transfer counts, and liveness are classified from the canonical board-wrapped functional_sim run. Define checker inputs, outputs, failure signatures, and repair boundaries without creating a separate smoke-acceptance path.
</task>
...
<subtask>
{
  ...
  "action_type": "functional_simulation_gate_refinement",
  "agent": "multilayer_pipeline_simulation_architect",
  "artifact_focus": [
    ...
    "gates: case_multilayer_pipeline, functional_sim, case_multilayer_functional, case_pipeline_deadlock_check"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "artifact.stage6.refined_multilayer_pipeline_subdag",
    "artifact.stage6.pipeline_simulation_evidence_contract",
    "artifact.stage6.pipeline_liveness_classifier_contract",
    "artifact.stage6.pipeline_failure_signature_map"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "verification_artifacts.multilayer_pipeline_simulation_architect",
  "objective": "Refine the third-layer multi-layer pipeline branch so that case_multilayer_pipeline is constructed only from promoted single-layer evidence and its functionality, ordering, transfer counts, and liveness are classified fr...<len=395>",
  "role": "multi-layer dataflow verification engineer",
  "role_assignment": {
    "collaboration_interfaces": {
      "acceptance_checker": "functional_sim",
      "consumes": [
        "artifact.stage3.pipeline_plan",
        "artifact.stage5.generated_code_package",
    ...
    "decision_authority": [
      "May refine dependencies and evidence schemas for multi-layer pipeline and liveness nodes.",
      "May reject smoke-only, identity-mismatched, or incomplete simulator evidence.",
      "May block third-layer promotion on ordering, count, or liveness failure but may not alter numeric policy or AXI address layout."
    ],
    "mission": "Make the multi-layer pipeline subgates executable, traceable, and derived from real board-wrapped simulation evidence.",
    "out_of_scope": [
      "Revalidating operator-leaf arithmetic without a contradictory trace.",
      "Defining AXI register maps or DDR packing.",
      "Running Vivado or board runtime."
    ],
    "primary_responsibilities": [
      "Bind layer ordering and stream boundaries to the Stage 3 pipeline plan.",
      "Specify valid/ready, transaction, tile, logical-index, transfer-count, and latency evidence required from functional_sim.",
      "Define deadlock-watchdog termination, progress counters, and liveness pass criteria.",
      "Ensure case_multilayer_functional and case_pipeline_deadlock_check consume the canonical simulation identity.",
      "Map failures to the earliest violated pipeline boundary and a bounded replay request."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Refine the multi-layer pipeline and liveness subgates"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.multilayer_pipeline_simulation_architect",
