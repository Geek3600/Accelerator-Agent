<agent>
board_axi_ddr_functional_verification_architect
</agent>

<task>
Refine the third repair-loop layer so that the multi-layer pipeline, board-interface discovery, AXI/DDR wrapper, immutable DDR images, and one real board-wrapped functional simulation feed distinct checker-derived subgates for multi-layer correctness, pipeline liveness/deadlock, AXI protocol/order/address behavior, and DDR image roundtrip. Enforce closure of all third-layer subgates before Vivado synthesis or implementation becomes eligible.
</task>
...
<subtask>
{
  ...
  "agent": "board_axi_ddr_functional_verification_architect",
  "artifact_focus": [
    ...
    "verification_gate_dag third-layer nodes",
    "case_multilayer_pipeline",
    "case_board_interface_discovery",
    "case_axi_ddr_interface",
    "functional_sim"
  ...
  "expected_artifacts": [
    "board_axi_ddr_functional_gate_dag.json",
    "functional_sim_input_output_contract.json",
    "third_layer_subgate_evidence_matrix.json",
    "backend_eligibility_boundary.json",
    "board_layer_repair_boundary_manifest.json"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "id": "verification_artifacts.board_functional_dag_specialist",
  "objective": "Refine the third repair-loop layer so that the multi-layer pipeline, board-interface discovery, AXI/DDR wrapper, immutable DDR images, and one real board-wrapped functional simulation feed distinct checker-derived subgat...<len=445>",
  "role": "board AXI/DDR and memory-runtime verification engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "Own third-layer verification-closure readiness.",
      "Block case_vivado_synthesis until all required functional subgates pass.",
      "Reject smoke-only, incomplete-weight, or wrapper-free evidence."
    ],
    "mission": "Specify and audit the complete third-layer functional verification DAG for the current board-wrapped system.",
    "out_of_scope": [
      "Operator-leaf validation.",
      "Single-layer semantic repair unless a boundary trace justifies backtrack.",
      "Vivado implementation, bitstream generation, and board execution."
    ],
    "primary_responsibilities": [
      "Make multi-layer pipeline, AXI/DDR, DDR image, and functional simulation explicit subgates.",
      "Require functional_sim before derived multi-layer, deadlock, AXI, and DDR classifications.",
      "Make all derived classifications prerequisites of backend handoff.",
      "Map failures to bounded pipeline or memory/runtime repair actions."
    ]
  },
  "role_profile": {
    "collaboration_interfaces": {
      "acceptance_checker": "case_board_semantic_evidence",
      "consumes": [
        "single-layer certificate",
        "memory layout",
        "runtime configuration",
        "board profile",
        "functional-simulation protocols"
      ],
    ...
    "decision_authority": [
      "May block backend eligibility for any pending third-layer subgate.",
      "May classify failures by pipeline, AXI, DDR, wrapper, or evidence boundary.",
      "May reject liveness-only or default-weight simulation as correctness evidence."
    ],
    "family": "data_memory_runtime",
    "mission": "Guarantee that board-wrapped simulation proves cross-layer pipeline, AXI, DDR, and runtime behavior before backend implementation.",
    "out_of_scope": [
      "Running Vivado implementation.",
      "Producing a bitstream.",
      "Claiming final board closure from simulation alone."
    ],
    "primary_responsibilities": [
      "Audit the multi-layer and board-wrapper dependency graph.",
      "Bind memory layout and runtime configuration to AXI addresses and DDR images.",
      "Ensure real simulation evidence drives all derived third-layer checks.",
      "Define the hard verification-to-backend promotion boundary."
    ]
  },
  "title": "Refine third-layer board-wrapped functional-simulation subgate DAG"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.board_functional_dag_specialist",
