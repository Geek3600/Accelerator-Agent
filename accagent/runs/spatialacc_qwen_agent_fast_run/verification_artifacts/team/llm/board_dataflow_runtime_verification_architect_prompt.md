<agent>
board_dataflow_runtime_verification_architect
</agent>

<task>
Define and audit the third repair-loop layer as one board AXI/DDR wrapped-system closure whose internal subgates cover multi-layer pipeline construction, board-interface discovery, AXI/DDR wrapping, real functional simulation, multi-layer numeric correctness, deadlock/liveness, AXI protocol/order/address behavior, and real DDR image roundtrip. Ensure every derived subgate depends on the same immutable board-wrapped simulator evidence and that all third-layer gates pass before Vivado synthesis is released.
</task>
...
<subtask>
{
  ...
  "agent": "board_dataflow_runtime_verification_architect",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    "artifact.stage6.board_axi_ddr_functional_gate_slice",
    "artifact.stage6.functional_sim_evidence_fanout_contract",
    "artifact.stage6.pre_vivado_verification_closure_contract"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.verification_closure_evidence_gate_auditor"
  ],
  "id": "verification_artifacts.board_axi_ddr_functional_dag_specialist",
  "objective": "Define and audit the third repair-loop layer as one board AXI/DDR wrapped-system closure whose internal subgates cover multi-layer pipeline construction, board-interface discovery, AXI/DDR wrapping, real functional simul...<len=510>",
  "role": "board AXI/DDR functional verification engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May reject smoke-only, liveness-only, stale-hash, or incomplete-weight simulator evidence.",
      "May block Vivado handoff when any third-layer subgate is pending or failed.",
      "May request memory/runtime or pipeline repair with a localized trace, but may not claim backend or board closure."
    ],
    "mission": "Make the board AXI/DDR wrapped system the definitive third-layer functional gate and keep backend execution downstream of complete simulator-based closure.",
    "out_of_scope": [
      "Operator-leaf implementation changes without localized evidence",
      "Vivado synthesis, implementation, route, or bitstream execution",
      "Physical board runtime acceptance"
    ],
    "primary_responsibilities": [
      "Bind the multi-layer pipeline, board interface, AXI/DDR wrapper, memory layout, and runtime configuration into one reproducible simulation target.",
      "Require immutable real inputs, complete Transformer-block weights, independent target-model boundary goldens, and explicit numeric tolerances.",
      "Define fanout from functional_sim evidence to multi-layer, deadlock/liveness, AXI protocol, address-map, transfer-count, and DDR roundtrip checkers.",
      "Verify that board-interface discovery precedes wrapper construction and simulation.",
      "Encode a hard dependency from all third-layer subgate passes to case_vivado_synthesis."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Refine the third-layer board-wrapped functional gate slice"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.board_axi_ddr_functional_dag_specialist",
