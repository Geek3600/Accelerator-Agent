<agent>
lower_layer_evidence_reuse_architect
</agent>

<task>
Refine the operator-leaf and connected single-transformer-layer portions of the real-tool DAG. Bind each promotion to immutable real-weight, target-reference, semantic-testbench, functional-simulation, golden-comparison, artifact-identity, and boundary-contract evidence. Reuse the existing operator-leaf and single-layer promotion certificates unless a current higher-layer boundary trace provides contradictory evidence; define the exact targeted-backtrack condition without reopening passed layers speculatively.
</task>
...
<subtask>
{
  ...
  "agent": "lower_layer_evidence_reuse_architect",
  "artifact_focus": [
    ...
    "operator leaf gates: case_real_weight_artifacts through case_operator_leaf_semantic_evidence",
    "single-layer gates: case_tb_scaffold through case_single_layer_semantic_evidence"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "artifact.stage6.refined_lower_layer_gate_subdag",
    "artifact.stage6.lower_layer_evidence_reuse_policy",
    "artifact.stage6.lower_layer_backtrack_trigger_contract",
    "artifact.stage6.lower_layer_checker_to_evidence_map"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.gate_dag_evidence_auditor"
  ],
  "id": "verification_artifacts.lower_layer_evidence_reuse_architect",
  "objective": "Refine the operator-leaf and connected single-transformer-layer portions of the real-tool DAG. Bind each promotion to immutable real-weight, target-reference, semantic-testbench, functional-simulation, golden-comparison,...<len=515>",
  "role": "operator-leaf and single-layer verification architect",
  "role_assignment": {
    "collaboration_interfaces": {
      "acceptance_checker": "required_real_tool_evidence_check",
      "consumes": [
    ...
    "decision_authority": [
      "May reorder or add dependencies within the operator-leaf and single-layer sub-DAG when required for cross-layer consistency.",
      "May classify existing promotion evidence as reusable, challenged, or insufficient.",
      "May request bounded targeted replay, but may not authorize free-form RTL repair or invalidate a certificate without contradictory evidence."
    ],
    "mission": "Produce a checker-bound lower-layer gate contract that preserves trusted leaf and single-layer closure while allowing only evidence-driven targeted backtracking.",
    "out_of_scope": [
      "Executing new RTL simulation unless a downstream executor consumes the plan.",
      "Designing the multi-layer AXI/DDR wrapper.",
      "Vivado synthesis, implementation, bitstream generation, or board runtime."
    ],
    "primary_responsibilities": [
      "Audit operator ordering, GQA/head mapping, tensor shapes, numeric tolerances, and template bindings represented by lower-layer evidence.",
      "Map real-weight, target-reference, DUT weight-consumption, simulator, golden, and immutable artifact evidence to each promotion gate.",
      "Confirm that single-layer execution consumes promoted leaf evidence and cannot bypass the semantic aggregate.",
      "Define the contradictory-boundary-trace threshold that permits targeted lower-layer replay.",
      "Prevent smoke results or default weights from being classified as correctness evidence."
    ]
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Refine lower-layer promotion and evidence-reuse gates"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.lower_layer_evidence_reuse_architect",
