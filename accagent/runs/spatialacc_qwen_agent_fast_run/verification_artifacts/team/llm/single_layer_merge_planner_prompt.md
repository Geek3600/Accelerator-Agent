<agent>
single_layer_merge_planner
</agent>

<task>
Refine the single-transformer-layer kernel verification gate sequence from verified leaves through real-weight testbench scaffold, layer functional simulation, golden comparison, and bounded failure slicing.
</task>
...
<subtask>
{
  ...
  "action_type": "cross_layer_merge_gate_planning",
  "agent": "single_layer_merge_planner",
  "artifact_focus": [
    "attention_semantics:gqa_rope_causal",
    "merge.self_attention",
    "merge.mlp",
    "merge.single_decoder_block",
    "gates:case_real_weight_artifacts,case_tb_scaffold,single_transformer_layer,case_single_layer_functional,case_single_layer_golden_compare"
  ],
  "constraints": [
  ...
  "expected_artifacts": [
    "single_layer_gate_subdag.json",
    "single_layer_merge_contract.json",
    "single_layer_numeric_tolerance_plan.json",
    "single_layer_failure_slice_boundaries.json"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.board_axi_ddr_system_planner",
    "verification_artifacts.repair_boundary_evidence_auditor"
  ],
  "id": "verification_artifacts.single_layer_merge_planner",
  "objective": "Refine the single-transformer-layer kernel verification gate sequence from verified leaves through real-weight testbench scaffold, layer functional simulation, golden comparison, and bounded failure slicing.",
  "role": "single transformer-layer integration verification engineer",
  "role_assignment": {
    "collaboration_interfaces": {
      ...
      "consumes": [
        "operator_leaf_gate_matrix",
        "leaf_boundary_contract_map",
        "artifact.stage3.pipeline_plan.attention_semantics",
        "artifact.stage4.parameter_binding.numeric_binding_plan",
        "artifact.stage5.generated_code_package",
        "candidate_stage_artifact.hierarchical_verification.merge_agents"
      ],
      ...
      "produces": [
        ...
        "single_layer_numeric_tolerance_plan",
        "single_layer_failure_slice_boundaries"
      ]
    },
    "decision_authority": [
      "May reject a single-layer merge if leaf evidence is incomplete or if operator ordering/head mapping conflicts with SACG model constraints.",
      "May require real weight/input materialization before functional simulation gates.",
      "May define failure localization boundaries between self-attention, MLP, residual, and norm regions but may not repair RTL directly."
    ],
    "mission": "Plan the second repair-loop layer that merges verified leaf modules into one transformer decoder-block kernel while preserving Qwen2 GQA attention semantics, residual ordering, numeric casts, and real-weight testbench requirements.",
    "out_of_scope": [
      "Do not plan multilayer/AXI/DDR implementation details beyond handoff requirements.",
      "Do not accept liveness-only simulator labels as functional correctness evidence.",
      "Do not claim Vivado or board-runtime readiness."
    ],
    "primary_responsibilities": [
      "Consume the operator leaf gate matrix, Stage 3 attention semantics, Stage 4 numeric binding plan, and Stage 5 generated accelerator package.",
      "Validate merge.self_attention, merge.mlp, and merge.single_decoder_block dependencies and ensure single_transformer_layer promotion occurs only after leaf golden evidence and real weight artifacts are available.",
      "Bind case_real_weight_artifacts, case_tb_scaffold, single_transformer_layer, case_single_layer_functional, and case_single_layer_golden_compare into the DAG with deterministic hashes and numeric tolerance references.",
      "Audit GQA head mapping, RoPE, causal ordering, residual_add_1/residual_add_2 placement, and fp16/fp32 cast points across the merged layer.",
      "Produce a single-layer merge readiness contract consumed by the board AXI/DDR system planner."
    ],
    "professional_family": "model_shape"
  },
  "role_profile": {
    "capabilities": [
  ...
  "title": "Plan single transformer-layer merge gates"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.single_layer_merge_planner",
