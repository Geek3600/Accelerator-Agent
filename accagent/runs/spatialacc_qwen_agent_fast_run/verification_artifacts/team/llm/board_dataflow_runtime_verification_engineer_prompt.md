<agent>
board_dataflow_runtime_verification_engineer
</agent>

<task>
Refine the third repair-loop layer so a current valid single-layer semantic certificate enables multi-layer pipeline construction, board-interface discovery, and AXI/DDR wrapper binding, followed by one canonical real board-wrapped functional simulation. Define multi-layer functionality, deadlock/liveness, AXI protocol/order/address behavior, and DDR image roundtrip as derived subgates of that same immutable run, then aggregate them in case_board_semantic_evidence before any Vivado backend node becomes eligible.
</task>
...
<subtask>
{
  ...
  "action_type": "board_wrapped_functional_gate_planning",
  "agent": "board_dataflow_runtime_verification_engineer",
  "artifact_focus": [
  ...
  "expected_artifacts": [
    ...
    "artifact.stage6.canonical_board_simulation_run_contract",
    "artifact.stage6.third_layer_classifier_binding_matrix",
    "artifact.stage6.board_failure_to_repair_routes",
    "artifact.stage6.backend_blocking_edge_manifest"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "verification_artifacts.repair_certificate_specialist",
    "verification_artifacts.canonical_gate_dag_auditor",
    "downstream.backend_board"
  ],
  "id": "verification_artifacts.board_axi_ddr_gate_specialist",
  "objective": "Refine the third repair-loop layer so a current valid single-layer semantic certificate enables multi-layer pipeline construction, board-interface discovery, and AXI/DDR wrapper binding, followed by one canonical real bo...<len=517>",
  "role": "board AXI/DDR wrapped-system verification engineer",
  "role_assignment": {
    ...
    "decision_authority": [
      "May block backend eligibility when any third-layer subgate is missing, stale, smoke-only, or tied to a different simulation identity.",
      "May classify failures into pipeline, wrapper, AXI, DDR, semantic, or tool-evidence repair domains.",
      "May not claim Vivado, bitstream, deployment, or board-runtime success."
    ],
    "mission": "Specify a production board-wrapped functional verification gate that proves multi-layer, AXI, DDR, and semantic correctness before backend eligibility.",
    "out_of_scope": [
      "Running Vivado synthesis or implementation",
      "Generating a runtime bitstream",
      "Executing board_runtime or accepting liveness-only smoke evidence"
    ],
    "primary_responsibilities": [
      "Consume current single-layer semantic promotion, generated AXI/DDR wrapper, memory layout, runtime configuration, semantic testbench, real weights, and board-interface discovery policy.",
      "Ensure interface discovery precedes wrapper-target binding and that no shell target is guessed.",
      "Bind one canonical functional_sim run to immutable input, complete block weights, wrapper identity, runtime artifacts, target-model reference, and output capture.",
      "Define multi-layer, liveness, AXI, and DDR classifiers as subgates derived from the same simulator evidence.",
      "Create backend-blocking edges and failure-localization routes for every third-layer failure class."
    ]
  },
  "role_profile": {
    "capabilities": [
      "audit data order",
      "check transfer-count and liveness risks",
      "bind DDR and runtime layout",
      "audit AXI protocol and address behavior",
      "reconcile classifiers from canonical simulation evidence"
    ],
    "constraint_focus": [
      "constraint.stream.order",
      "constraint.beat.pipeline",
  ...
  "title": "Refine the third-layer board-wrapped functional gate sub-DAG"
}
</subtask>
...
<candidate_stage_artifact>
{
  ...
  "role_slice_policy": {
    ...
    "sliced_for_subtask": "verification_artifacts.board_axi_ddr_gate_specialist",
