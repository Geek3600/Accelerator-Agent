...
<task>
Integrate specialist reviews, confirm that all current-stage acceptance items are checker-backed, classify remaining items as later-stage obligations rather than risks when appropriate, and enforce SACG repair boundaries for any failed or missing evidence.
</task>
...
<subtask>
{
  ...
  "expected_artifacts": [
    ...
    "consolidated pass/fail matrix by checker and SACG constraint",
    "bounded repair or backtrack requests if any specialist finding blocks promotion"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "next_stage.parameter_binding_or_memory_layout",
    "stage_repair.pipeline_planning"
  ],
  "id": "pipeline_planning.evidence_gate_auditor",
  "objective": "Integrate specialist reviews, confirm that all current-stage acceptance items are checker-backed, classify remaining items as later-stage obligations rather than risks when appropriate, and enforce SACG repair boundaries...<len=256>",
  "role": "verification evidence gate auditor",
  "title": "Aggregate checker evidence and enforce repair boundaries"
}
</subtask>
