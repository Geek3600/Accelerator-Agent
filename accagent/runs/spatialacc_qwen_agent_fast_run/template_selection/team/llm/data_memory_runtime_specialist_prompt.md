...
<task>
Audit the proposed FFN tile bindings against board memory, runtime layout, stream order, lanes, numeric bit widths, and transfer/liveness risks so template parameters remain cross-layer consistent with deployment constraints.
</task>
...
<subtask>
{
  ...
  "expected_artifacts": [
    "artifact.template_selection.tile_memory_runtime_consistency_review",
    "tile-to-memory/runtime consistency matrix with cited SACG facts",
    "list of any board/runtime contradictions or not-current-stage risks"
  ],
  "handoff_rule": {
    "completion_criteria": [],
  ...
  "handoff_to": [
    "template_selection.ffn_tile_binding_engineer",
    "template_selection.verification_repair_gate"
  ],
  "id": "template_selection.data_memory_runtime_tile_auditor",
  "objective": "Audit the proposed FFN tile bindings against board memory, runtime layout, stream order, lanes, numeric bit widths, and transfer/liveness risks so template parameters remain cross-layer consistent with deployment constra...<len=225>",
  "role": "memory and runtime tile consistency auditor",
  "title": "Check board and runtime consistency of FFN tile bindings"
}
</subtask>
