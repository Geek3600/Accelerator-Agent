#!/usr/bin/env python3
"""Check case hierarchical verification evidence gates.
...
def result(gate: str, checks: list[dict[str, Any]], blockers: list[str], run_dir: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "spatialaccagent.case_hierarchical_check.v0",
        "gate": gate,
...
def check_stage_leaf_static(run_dir: Path) -> dict[str, Any]:
    ...
    for path in [top_path, fpga_top_path]:
        if path.exists():
...
def manifest_candidates(run_dir: Path) -> list[Path]:
    candidates = []
    env_manifest = os.environ.get("SPATIALACC_REAL_WEIGHT_MANIFEST")
    if env_manifest:
        candidates.append(Path(env_manifest))
    candidates.extend(
        [
            run_dir / "verification" / "model_weights" / "weight_manifest.json",
            run_dir / "verification" / "case_real_weights" / "weight_manifest.json",
            run_dir / "generated" / "memory" / "case_real_weight_manifest.json",
...
def check_leaf_evidence_gate(run_dir: Path, gate: str) -> dict[str, Any]:
    ...
    for stage_id in stage_ids:
        static_candidates = [
            ...
            run_dir / "verification" / "case_hierarchy" / f"stage_leaf_static__{safe_id(stage_id)}.json",
            run_dir / "verification" / "case_hierarchy" / "stage_leaf_static.json",
        ]
...
def check_runtime_bitstream(run_dir: Path) -> dict[str, Any]:
    checks: list[dict[str, Any]] = []
    blockers: list[str] = []
    app_shell_report_path = (
        run_dir / "backend_board" / "case_diagnostics" / "app_shell_runtime_vivado.json"
    )
    app_shell_report = read_json(app_shell_report_path) if app_shell_report_path.is_file() else {}
    reported_bitstream = Path(str(app_shell_report.get("bitstream_path") or ""))
    bit_candidates = [
        reported_bitstream,
        run_dir / "app_shell_runtime_bitstream" / "app_shell_runtime.bit",
        run_dir / "app_shell_runtime_bitstream" / "app_shell.bit",
    ]
    found_runtime_bit = any(path.is_file() and path.stat().st_size > 0 for path in bit_candidates)
    checks.append(
        {
            "name": "exact_app_shell_runtime_report",
            "path": str(app_shell_report_path),
            "report_status": app_shell_report.get("status"),
            "status": "pass" if app_shell_report.get("status") == "pass" else "fail",
        }
    )
    checks.append(
        {
            "name": "exact_app_shell_bitstream",
            "candidates": [str(path) for path in bit_candidates],
            "status": "pass" if found_runtime_bit else "fail",
        }
    )
    if app_shell_report.get("status") != "pass":
        blockers.append("exact app-shell Vivado runtime report is not pass")
    if not found_runtime_bit:
        blockers.append("exact app-shell runtime bitstream is missing")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check case hierarchical verification evidence gates")
...
def main(argv: list[str] | None = None) -> int:
    ...
    out_dir = run_dir / "verification" / "case_hierarchy"
    ...
    if args.gate == "all":
        summary = {
            "schema_version": "spatialaccagent.case_hierarchical_check_summary.v0",
