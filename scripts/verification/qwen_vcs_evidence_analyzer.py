#!/usr/bin/env python3
"""Analyze Qwen VCS functional-sim evidence and emit repair handoff JSON."""
...
PROGRESS_RE = re.compile(r"qwen_axi_board_tb progress (?P<fields>.*)")
FATAL_RE = re.compile(r"qwen_axi_board_tb (?P<kind>NO_PROGRESS|TIMEOUT|FAIL)(?P<fields>.*)")
PASS_RE = re.compile(r"qwen_axi_board_tb PASS real functional ddr path (?P<fields>.*)")
...
def load_tool_text(run_dir: Path) -> tuple[str, list[str]]:
    ...
    legacy_paths = [
        run_dir / "verification" / "real_tools" / "qwen_vcs_functional_sim.json",
        run_dir / "verification" / "real_tools" / "qwen_vcs_smoke.json",
    ]
    ...
    legacy_logs = [
        run_dir / "verification" / "vcs" / "qwen_functional_sim.log",
    ]
...
def write_pipeline_boundary_trace(
    ...
    counters = [
        ...
        ("pipe_l0_out_beats", "", "boundary.multilayer.layer_0_to_1", "QwenMultiLayerSystemTop.layer_0_to_1"),
        ("pipe_l1_out_beats", "", "boundary.multilayer.layer_1_to_2", "QwenMultiLayerSystemTop.layer_1_to_2"),
        ("pipe_probe_a_beats", "", "boundary.multilayer.quarter_probe", "QwenMultiLayerSystemTop.quarter_probe"),
        ("pipe_probe_b_beats", "", "boundary.multilayer.midpoint_probe", "QwenMultiLayerSystemTop.midpoint_probe"),
        ("pipe_probe_c_beats", "", "boundary.multilayer.three_quarter_probe", "QwenMultiLayerSystemTop.three_quarter_probe"),
        ("pipe_last_out_beats", "", "boundary.multilayer.last_layer_output", "QwenMultiLayerSystemTop.last_layer_output"),
        ("output_valid_beats", "out_valid", "boundary.board.output_valid", "QwenAxiBoardSystemTop.output_capture"),
...
def repair_patterns(root_cause: str) -> list[dict[str, Any]]:
    patterns: dict[str, list[dict[str, Any]]] = {
        "mlp_branch_join_no_output": [
            {
                "pattern_id": "mlp_branch_join_skid_buffers",
                "target_files": ["accagent/framework/templates/operator_chisel/FFN.scala"],
                "summary": "Insert small queues after GatedMLP gate/activation and up branches before ElementwiseMul so branch-latency skew cannot deadlock the join.",
                "approval_required": False,
                "risk": "low_resource_delta_small_fifo",
            },
            {
                "pattern_id": "rerun_stage5_codegen_after_template_repair",
                "target_files": ["accagent/framework/stage_artifacts.py"],
                "summary": "Regenerate the Chisel package and rerun elaboration/static compile after template repair.",
                "approval_required": False,
                "risk": "required_to_refresh_generated_rtl",
            },
        ],
        "mlp_no_output_after_branch_skid_buffers": [
            {
                "pattern_id": "gated_mlp_branch_state_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Skid buffers are already present; expose gate/up/actQ/upQ/mul/down valid/ready/state signals to localize the remaining GatedMLP non-output condition.",
                "approval_required": False,
                "risk": "testbench_only",
            },
            {
                "pattern_id": "gated_mlp_branch_contract_check",
                "target_files": ["accagent/framework/templates/operator_chisel/FFN.scala", "accagent/framework/templates/operator_chisel/Elementwise.scala"],
                "summary": "Check GatedMLP branch stream contracts after queues, especially up branch last propagation, down Linear input last, and ElementwiseMul lhs/rhs alignment.",
                "approval_required": False,
                "risk": "template_semantics_repair",
            },
        ],
        "gated_mlp_gate_branch_no_output": [
            {
                "pattern_id": "gated_mlp_gate_linear_state_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py", "accagent/framework/templates/operator_chisel/Linear.scala"],
                "summary": "GatedMLP input is accepted but gate branch produces no handshakes; expose gate Linear state/counters and check tile/input last contract.",
                "approval_required": False,
                "risk": "testbench_first_template_if_confirmed",
            }
        ],
        "gated_mlp_up_branch_no_output": [
            {
                "pattern_id": "gated_mlp_up_linear_state_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py", "accagent/framework/templates/operator_chisel/Linear.scala"],
                "summary": "GatedMLP gate branch progresses but up branch produces no handshakes; expose up Linear state/counters and check branch input ready coupling.",
                "approval_required": False,
                "risk": "testbench_first_template_if_confirmed",
            }
        ],
        "gated_mlp_activation_no_output": [
            {
                "pattern_id": "gated_mlp_activation_contract_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py", "accagent/framework/templates/operator_chisel/Activation.scala"],
                "summary": "GatedMLP gate branch produces output but activation/actQ path does not handshake; inspect activation valid/ready and last propagation.",
                "approval_required": False,
                "risk": "testbench_first_template_if_confirmed",
            }
        ],
        "gated_mlp_mul_join_no_output": [
            {
                "pattern_id": "gated_mlp_mul_join_contract_repair",
                "target_files": ["accagent/framework/templates/operator_chisel/Elementwise.scala", "accagent/framework/templates/operator_chisel/FFN.scala"],
                "summary": "Both queued GatedMLP branches are present but ElementwiseMul emits no output; inspect lhs/rhs ready coupling and branch alignment.",
                "approval_required": False,
                "risk": "template_semantics_repair",
            }
        ],
        "gated_mlp_down_linear_no_output": [
            {
                "pattern_id": "gated_mlp_down_linear_contract_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py", "accagent/framework/templates/operator_chisel/Linear.scala"],
                "summary": "ElementwiseMul emits handshakes but down Linear does not produce output; inspect down Linear state, input last, and output ready path.",
                "approval_required": False,
                "risk": "testbench_first_template_if_confirmed",
            }
        ],
        "residual_add2_join_or_output_stall": [
            {
                "pattern_id": "residual_add2_state_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "GatedMLP down output is accepted, but the decoder layer still produces no output; expose add2 residual/computed/output and res2Q valid/ready counters.",
                "approval_required": False,
                "risk": "testbench_only",
            },
            {
                "pattern_id": "residual_add2_contract_check",
                "target_files": ["accagent/framework/templates/operator_chisel/DecoderBlock.scala", "accagent/framework/templates/operator_chisel/Residual.scala"],
                "summary": "Check post-MLP residual join stream contract, especially res2Q enqueue/dequeue, add2 output ready, and last propagation.",
                "approval_required": False,
                "risk": "template_semantics_repair",
            },
        ],
        "multilayer_pipeline_or_top_output_propagation_stall": [
            {
                "pattern_id": "multilayer_pipeline_progress_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Layer-0 emits accepted beats, but the top output is still absent; expose accepted-beat counters at pipeline links 0->1, 1->2, and last->DDR.",
                "approval_required": False,
                "risk": "testbench_only",
            },
            {
                "pattern_id": "watchdog_internal_progress_accounting",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Treat internal pipeline link handshakes as progress so the watchdog can distinguish a live deep pipeline from a real deadlock; final acceptance still requires top-level DDR writes.",
                "approval_required": False,
                "risk": "testbench_diagnostic_policy",
            },
        ],
        "deep_pipeline_latency_or_downstream_layer_stall": [
            {
                "pattern_id": "multilayer_deep_pipeline_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Early pipeline links are producing beats but the last link is silent; expose quarter/mid/three-quarter pipeline link counters to locate the first non-forwarding layer range.",
                "approval_required": False,
                "risk": "testbench_only",
            },
            {
                "pattern_id": "functional_timeout_budget_from_pipeline_depth",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Derive functional-sim max cycles from model layer count instead of a fixed short window, while still requiring real top-level DDR output for pass.",
                "approval_required": False,
                "risk": "testbench_diagnostic_policy",
            },
        ],
        "top_output_capture_or_axi_write_stall": [
            {
                "pattern_id": "top_output_axi_write_probe",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "The final pipeline link emits beats but DDR writes do not occur; expose top output valid/ready and AXI write FSM counters.",
                "approval_required": False,
                "risk": "testbench_or_runtime_wrapper",
            }
        ],
        "transaction_last_granularity_mismatch": [
            {
                "pattern_id": "vector_transaction_last_per_token",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Drive st/addr/last at hidden-vector transaction granularity instead of whole-sequence granularity.",
                "approval_required": False,
                "risk": "verification_wrapper_only",
            }
        ],
        "first_layer_no_output": [
            {
                "pattern_id": "layer0_internal_probe_required",
                "target_files": ["scripts/verification/qwen_tb_generator.py"],
                "summary": "Expose layer-0 internal valid/ready probes to localize the first non-producing operator.",
                "approval_required": False,
                "risk": "testbench_only",
            }
        ],
        "real_ddr_path_not_exercised": [
            {
                "pattern_id": "bind_real_memh_plusargs",
                "target_files": ["scripts/verification/qwen_weight_manifest.py", "scripts/verification/qwen_tb_generator.py"],
                "summary": "Generate readmemh DDR images and require TB plusargs/readmemh to consume real input and weight artifacts.",
                "approval_required": False,
                "risk": "verification_artifact_path_change",
            }
        ],
    }
    return patterns.get(root_cause, [])


def has_mlp_branch_skid_buffers(run_dir: Path) -> bool:
    candidates = [
        Path("accagent/framework/templates/operator_chisel/FFN.scala"),
        run_dir / "generated" / "chisel" / "GatedMLP.sv",
        run_dir / "generated" / "chisel" / "src" / "main" / "scala" / "spatialaccagent" / "templates" / "FFN.scala",
    ]
    for path in candidates:
        if not path.exists():
            continue
        text = path.read_text(encoding="utf-8", errors="ignore")
        if "actQ" in text and "upQ" in text:
            return True
    return False


def classify(progress: list[dict[str, Any]], fatal: dict[str, Any] | None, sim_pass: bool, run_dir: Path) -> tuple[str, str]:
    if sim_pass:
        return "none", "functional simulation passed with real DDR path evidence"
    if not progress and not fatal:
        return "missing_sim_log", "VCS tool did not emit parseable qwen_axi_board_tb progress or fatal evidence"
    last = latest(progress)
    ...
    if input_beats == 0 or weight_beats == 0:
        return "real_ddr_path_not_exercised", "functional sim did not consume both input and weight DDR images"
    if output_beats > 0:
        return "output_started_but_completion_failed", "top-level output appeared but completion/done evidence failed"
    pipe_l0_beats = max_counter(progress, "pipe_l0_out_beats")
    pipe_l1_beats = max_counter(progress, "pipe_l1_out_beats")
    pipe_last_beats = max_counter(progress, "pipe_last_out_beats")
    pipe_probe_a_beats = max_counter(progress, "pipe_probe_a_beats")
    pipe_probe_b_beats = max_counter(progress, "pipe_probe_b_beats")
    pipe_probe_c_beats = max_counter(progress, "pipe_probe_c_beats")
    if pipe_last_beats > 0:
        return "top_output_capture_or_axi_write_stall", "The final pipeline link emits accepted beats, but top-level output/AXI write evidence remains absent"
    if pipe_probe_c_beats > 0 or pipe_probe_b_beats > 0:
        return "deep_pipeline_latency_or_downstream_layer_stall", "Deep pipeline probes show early/mid-layer propagation but no final-link output before the current tool window ends"
    if pipe_probe_a_beats > 0 or pipe_l1_beats > 0:
        return "deep_pipeline_latency_or_downstream_layer_stall", "Early pipeline links are active, but later pipeline probes remain silent within the current tool window"
    if pipe_l0_beats > 0:
        return "multilayer_pipeline_or_top_output_propagation_stall", "Layer-0 output reaches the first pipeline link, but later pipeline propagation or top capture is absent"
    gate_beats = max_counter(progress, "mlp_gate_out_beats")
    up_beats = max_counter(progress, "mlp_up_out_beats")
    act_beats = max_counter(progress, "mlp_act_out_beats")
    actq_beats = max_counter(progress, "mlp_actq_deq_beats")
    upq_beats = max_counter(progress, "mlp_upq_deq_beats")
    mul_beats = max_counter(progress, "mlp_mul_out_beats")
    down_beats = max_counter(progress, "mlp_down_out_beats")
    add2_out_beats = max_counter(progress, "add2_out_beats")
    has_mlp_counters = any(f"mlp_{name}" in last for name in ["gate_out_beats", "up_out_beats", "act_out_beats", "actq_deq_beats", "upq_deq_beats", "mul_out_beats", "down_out_beats"])
    if has_mlp_counters and last.get("l0_rms2") == 1 and last.get("l0_mlp") == 0:
        if gate_beats == 0:
            return "gated_mlp_gate_branch_no_output", "GatedMLP probes are active and the gate branch produced no accepted output beats"
        if up_beats == 0:
            return "gated_mlp_up_branch_no_output", "GatedMLP gate branch progressed but up branch produced no accepted output beats"
        if act_beats == 0 or actq_beats == 0:
            return "gated_mlp_activation_no_output", "GatedMLP activation or actQ path did not produce accepted beats"
        if upq_beats == 0 or mul_beats == 0:
            return "gated_mlp_mul_join_no_output", "GatedMLP branch queues did not both drain through ElementwiseMul"
        if down_beats == 0:
            return "gated_mlp_down_linear_no_output", "GatedMLP ElementwiseMul produced accepted beats but down Linear emitted no accepted output beats"
        if add2_out_beats > 0:
            if pipe_last_beats > 0:
                return "top_output_capture_or_axi_write_stall", "The final pipeline link emits accepted beats, but top-level output/AXI write evidence remains absent"
            if pipe_l1_beats > 0:
                return "deep_pipeline_latency_or_downstream_layer_stall", "Early pipeline links are active, but the final pipeline link remains silent within the current functional-sim window"
            if pipe_l0_beats > 0:
                return "multilayer_pipeline_or_top_output_propagation_stall", "Layer-0 output reaches the first pipeline link, but layer-1 output or later propagation is absent"
            return "multilayer_pipeline_or_top_output_propagation_stall", "Layer-0 residual add2 emits accepted beats, but top-level output remains absent; failure moved to multi-layer propagation, deep-pipeline latency, or top-output capture"
        return "residual_add2_join_or_output_stall", "GatedMLP down output is accepted, but layer output remains absent; failure moved to residual add2 or layer-output join"
    if last.get("l0_rms2") == 1 and last.get("l0_mlp") == 0:
        if has_mlp_branch_skid_buffers(run_dir):
            return "mlp_no_output_after_branch_skid_buffers", "layer-0 reaches RMS2 and GatedMLP skid buffers are present, but GatedMLP still emits no output"
        return "mlp_branch_join_no_output", "layer-0 attention path reaches RMS2, but GatedMLP does not emit output"
    layer0 = last.get("layer_vr_0")
    layer1 = last.get("layer_vr_1")
    if isinstance(layer0, dict) and isinstance(layer1, dict) and layer0.get("valid") == 0 and layer1.get("valid") == 0:
        return "first_layer_no_output", "first decoder layer never produces a stream beat for layer 1"
    if fatal and str(fatal.get("kind")) in {"NO_PROGRESS", "TIMEOUT"}:
        return "functional_sim_no_output", "real DDR input/weight path ran, but no top-level output was observed before watchdog"
    return "unknown_functional_failure", "VCS evidence failed but no known repair pattern matched"
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "observed": observed,
        "repair_handoff": {
            "repair_patterns": repair_patterns(root_cause),
            "next_stage": "repair",
            "agent_should_apply_code_changes": bool(repair_patterns(root_cause)) and not sim_pass,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze Qwen VCS functional-sim logs and generate repair handoff")
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = args.out or run_dir / "verification" / "qwen_vcs" / "qwen_vcs_functional_diagnosis.json"
