#!/usr/bin/env python3
"""Generate Qwen multi-layer and AXI/DDR VCS scaffold files."""
...
DEFAULT_RUN_DIR = Path("/tmp/spatialacc_qwen_agent_fast_run")
...
def generated_top_params(run_dir: Path) -> dict[str, int]:
    ...
    text = top.read_text(encoding="utf-8", errors="ignore") if top.exists() else ""
    ...
    hidden_size = int(hidden_match.group(1)) if hidden_match else 896
    lanes = int(lanes_match.group(1)) if lanes_match else 8
    max_seq_len = int(seq_match.group(1)) if seq_match else 16
    return {
...
def model_layers(run_dir: Path) -> int:
    ...
    if not path.exists():
        return 24
    value = read_json(path).get("num_layers", 24)
    return int(value)
...
def multilayer_sv(params: dict[str, int], layers: int) -> str:
    ...
    pos_w = params["pos_w"]
    return f"""// Generated by qwen_tb_generator.py. Multi-layer stream wrapper scaffold.
...
def axi_wrapper_sv(params: dict[str, int], layers: int) -> str:
    ...
    return f"""// Generated by qwen_tb_generator.py. Qwen AXI/DDR wrapper scaffold.
...
def build_outputs(args: argparse.Namespace) -> dict[str, Any]:
    ...
    paths = {
        "run_multilayer": chisel_dir / "QwenMultiLayerSystemTop.sv",
        "repo_axi_wrapper": REPO_ROOT / "verification" / "rtl" / "QwenAxiBoardSystemTop.sv",
        "repo_tb": REPO_ROOT / "testbench" / "vcs" / "qwen_axi_board_tb.sv",
        "manifest": run_dir / "generated" / "verification" / "qwen_tb_generator_manifest.json",
    }
    ml_text = multilayer_sv(params, layers)
    write_text(paths["run_multilayer"], ml_text)
    if args.repo_multilayer_copy:
        write_text(REPO_ROOT / "verification" / "rtl" / "QwenMultiLayerSystemTop.sv", ml_text)
    write_text(paths["repo_axi_wrapper"], axi_wrapper_sv(params, layers))
    write_text(paths["repo_tb"], tb_sv(layers))
    manifest = {
        "schema_version": "spatialaccagent.qwen_tb_generator.v0",
        "run_dir": str(run_dir),
        "num_layers": layers,
        "generated_top_params": params,
        "outputs": {name: str(path) for name, path in paths.items() if name != "manifest"},
        "notes": [
            "QwenLayerPipelineTop chains GeneratedAcceleratorTop instances with ready/valid streams.",
            "QwenAxiBoardSystemTop is an AXI/DDR scaffold for VCS closure; runtime bitstream and board pass still require separate evidence.",
        ],
    }
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Qwen AXI/DDR VCS scaffold")
    parser.add_argument("--run-dir", type=Path, default=DEFAULT_RUN_DIR)
    parser.add_argument("--repo-multilayer-copy", action="store_true")
...
if __name__ == "__main__":
    try:
        ...
        print(f"qwen_tb_generator.py: {exc}", file=sys.stderr)
        raise SystemExit(1)
